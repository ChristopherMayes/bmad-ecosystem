...
\emph{Validation} note (which gate pins it). The physics was transcribed from Genesis
...
Eq.~\ref{eq:zglobal} by the migration phase-continuity gate ($6.9\times10^{-15}$) and
the seam-wake causality gate (exactly zero ahead of all charge).
\end{validation}
...
assertion gates refuse each malformed lattice by name; mutations (rms factor, wrong
...
\end{provenance}
\begin{validation}
tier1/td1 at the constants floor; the split-weight gate ($3.6\times10^{-13}$) pins the
...
\end{provenance}
\begin{validation}
Statistical gate $\langle|b|^2\rangle N_\lambda = 1.03\pm0.14$, uniform and
...
blind spot that fixed its sweep range.
\end{validation}
...
\emph{The energy ledger} (gate zero, written per record step to
...
as in Genesis and MINERVA); the gate level sits above it. The physical field of the
...
six digits, awfac), Runge-Kutta-Gill --- never a bit gate; no MINERVA code was
...
$1.0\times10^{-5}$ of the field-energy turnover (gate $10^{-4}$; degenerate with the
...
relative (gates $5\times10^{-3}$); step-size convergence $f_c(10/20/30$ per period$)
= 0.750582/0.750505/0.750502$; full-segment gain curve vs the averaged mode
$|\ln$ ratio$| = 2.6\times10^{-3}$ at exit (priced, gate $0.2$). Mutations: the
$E\cdot v$ sign fails the ledger at $2.0$ and the gain comparison at $0.69$ (the
magnitude-blind $f_c$ gates pass it --- why the ledger is gate zero); a hard-edge
entry fails the orbit-handoff gate at $3.2\times10^{-5}$ m ($19\sigma$; NOTE the
...
it, so results are bit-identical across thread counts --- a gated property, and the
...
thread gate and its shared-accumulator mutation (7.0 relative between thread counts,
