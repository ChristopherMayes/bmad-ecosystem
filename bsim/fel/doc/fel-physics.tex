...
\title{The Bmad FEL Tracker: Physics Manual}
\author{\code{bsim/fel} --- \code{fel\_beam\_mod}, \code{fel\_track\_mod},
...
(\code{bsim/fel/tests/}) proves each statement against Genesis 1.3 Version 4, against
closed forms, or against independent implementations, and the measured levels live in
\code{bsim/fel/README.md}. Each section ends with a \emph{Provenance} note (the Genesis
...
file (each also recorded for upstream in \code{bsim/fel/openpmd/UPSTREAM-NOTES.md}):
...
and the carried Python patch (\code{bsim/fel/openpmd/}, the class's future
