...
sentinel $-1$; silence never means hard edge. Refused by name at parse time: missing \code{b\_max} or \code{l\_period}, a
...
uses modal fields; it is a verification mode, run on a handful of slices where its
$\sim$30$\times$ cost is irrelevant. Nothing from the averaged coupling path may
...
= 0.750582/0.750505/0.750502$; full-segment gain curve vs the averaged mode
$|\ln$ ratio$| = 2.6\times10^{-3}$ at exit (priced, check $0.2$). Mutations: the
