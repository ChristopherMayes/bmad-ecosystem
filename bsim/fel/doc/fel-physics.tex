...
\begin{equation}\label{eq:unavgkick}
  ...
  \mathrm{src} \mathrel{+}= i\,e^{-i\Psi}\,\hat\jmath\;
     \frac{Z_0\, c\,\delta}{2\,\Delta_{\mathrm{grid}}^2\,\Delta s}\,\frac{w}{\gamma},
\end{equation}
the averaged deposit of Eq.~\ref{eq:source} with the coupling removed and the actual
quiver current in its place. Period-averaging this pair reproduces
\S\ref{sec:coupling} exactly --- the $JJ$ Bessel factor emerges from the figure-8 ---
and the pair is energy-consistent by construction to $O(1-\beta_\parallel)$, which is
what makes the ledger a gate on their mutual normalization. The physical field of the
...
transcribed. The magnetic push is explicit RK4 (MINERVA's production formulation),
not the brief's fully symplectic alternative; the ballistic gate prices the
difference (exact $\gamma$, emittance drift $10^{-10}$ over a probe).
\end{provenance}
\begin{validation}
\code{check\_unaveraged.py}, all self-referenced or closed-form: energy ledger
$6.7\times10^{-4}$ of the field-energy turnover (gate $2\times10^{-3}$); ledger
internal kick-vs-realized $1.0\times10^{-5}$; ballistic $\Delta\gamma$ exactly zero,
