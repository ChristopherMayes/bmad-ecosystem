...
unroll. Refused by name at parse time: missing \code{b\_max} or \code{l\_period}, a
...
\subsection{The Bmad-kernel alternative}\label{sec:undbmad}

\code{und\_transport = "bmad"} replaces \S\ref{sec:natfocus} by the transverse maps of
Bmad's periodic-wiggler kernel, flattened: per-particle $k_1 = -\tfrac12 g_{\max}^2/
...
tier2\_bmad $5.0\times10^{-2}$, the located and priced transport difference;
und\_transport pricing above.
\end{validation}
...
(\code{fel\_unaveraged\_mod}, \code{und\_transport = "unaveraged"}) integrates the
