...
Four panels against z: radiation power (log), bunching, energy change and spread
(both in units of m_e c^2, so one axis carries both honestly), and transverse rms
beam sizes. Works on steady-state and time-dependent files alike: with more than
one slice, thin gray lines show every slice and the bold line the total (power)
or the plain slice average (everything else).
...
def load(fn):
    """Return z (nrec,) and per-quantity arrays (nrec, nslice) in time order."""
    d = np.loadtxt(fn)
    if d.ndim == 1:
        d = d[None, :]
    nslice = int(d[:, 1].max())
    ...
    q = {name: d[:, :, i] for i, name in enumerate(
        ("z", "slice", "power", "on_axis", "bunching", "bunching_phase",
         "mean_gamma", "sigma_gamma", "sigma_x", "sigma_y"))}
    return d[:, 0, 0], q, nslice
...
def main():
    ...
    # Energy: change of the mean and the rms spread share the unit m_e c^2, so
    # one axis carries both (never two scales on one panel).
    dgamma = q["mean_gamma"] - q["mean_gamma"][0, :]
    panel_series(ax_g, z, dgamma, BLUE, label=r"$\Delta\langle\gamma\rangle$")
    panel_series(ax_g, z, q["sigma_gamma"], ORANGE, label=r"$\sigma_\gamma$")
    ax_g.set_ylabel(r"[$m_e c^2$]")
    ax_g.set_xlabel("z [m]")
    ax_g.set_title("Energy change and spread", loc="left")
    ax_g.legend(**above)
