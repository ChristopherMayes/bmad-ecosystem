...
read (d_unit, err = 9070, end = 9070) lat%branch(0)%name, lat%branch(0)%ix_fixer, lat%branch(0)%particle_start
...
if (found_it) then
  ...
  read (d_unit, iostat = ios4) ptc_com_read%max_fringe_order, ptc_com_read%exact_model, ptc_com_read%exact_misalign, &
          ...
          ptc_com_read%pancake_symplectic, ptc_com_read%pancake_canonical
  ...
  if (extra%pancake_symplectic_set)               ptc_com%pancake_symplectic                        = ptc_com_read%pancake_symplectic
  if (extra%pancake_canonical_set)                ptc_com%pancake_canonical                         = ptc_com_read%pancake_canonical
...
read (d_unit, err = 9100, end = 9100) &
        ...
        ele%aperture_type, ele%csr_method, ele%space_charge_method, ele%fel_method, ele%orientation, &
