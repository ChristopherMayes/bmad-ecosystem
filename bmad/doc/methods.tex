...
\item[\vn{Custom}]
This method can be used to call custom tracking code supplied by the User. See \vn{s:custom.ele}
for more details.
...
%                                     BS   C   L   M   RK    SLB   SLP    T   TRK
  ...
  \vn{beambeam}                      & D & X & X &   &     &     &     &     &    \\  
  \vn{bends: rbend and sbend}        & D & X & X & X &  X  &     &  X  &  X  & X  \\ 
  \vn{converter}                     & D & X &   &   &     &     &     &     &    \\  
...
\vn{s:custom.ele} for more details.
...
\begin{tabular}{lccccccccc} \toprule
\rule{0pt}{80pt} &
\begin{sideways}\vn{Auto}\end{sideways} &
\begin{sideways}\vn{Bmad_Standard}\end{sideways} &
\begin{sideways}\vn{Custom}\end{sideways} &
\begin{sideways}\vn{MAD}\end{sideways} &
...
\\ \midrule
%                                     A   BS   C   M  Stat SLB   SLP   Tlr  Trk
  \vn{ab_multipole and multipole}    & D & X & X &   & X &     &  X  &  X  & X \\  
  \vn{ac_kicker}                     &   &   & X &   & X &     &  X  &  X  & D \\  
  \vn{beambeam}                      & D & X & X &   & X &     &     &     & X \\  
  \vn{bends: rbend and sbend}        & D & X & X & X & X &     &  X  &  X  & X \\ 
  \vn{converter}                     & D & X & X &   &   &     &     &     &   \\  
  ...
  \vn{fiducial}                      & D & X & X &   & X &     &  X  &     & X \\  
  \vn{floor_shift}                   & D & X & X &   & X &     &  X  &     & X \\  
  \vn{hkicker}                       & D & X & X &   & X &     &  X  &  X  & X \\  
  \vn{instrument, monitor, and pipe} & D & X & X &   & X &     &  X  &  X  & X \\  
  \vn{kicker}                        & D & X & X &   & X &     &  X  &  X  & X \\  
  \vn{lcavity and rfcavity}          & D & X & X &   & X &     &  X  &  X  & X \\  
  \vn{marker}                        & D & X & X &   & X &     &  X  &  X  & X \\  
  \vn{match}                         & D & X & X &   & X &     &     &     & X \\  
  \vn{octupole}                      & D & X & X &   & X &     &  X  &  X  & X \\ 
  \vn{patch}                         & D & X & X &   & X &     &  X  &  X  & X \\ 
  \vn{quadrupole}                    & D & X & X & X & X &  X  &  X  &  X  & X \\ 
  \vn{rf_bend}                       & D & X & X &   & X &     &     &     & X \\
  \vn{sad_mult}                      & D & X & X &   & X &     &  X  &  X  & X \\ 
  \vn{sextupole}                     & D & X & X & X & X &     &  X  &  X  & X \\ 
  \vn{solenoid}                      & D & X & X & X & X &  X  &  X  &  X  & X \\ 
  \vn{sol_quad}                      & D & X & X & X & X &  X  &  X  &  X  & X \\ 
  \vn{taylor}                        &   &   & X &   & X &     &  X  &  D  &   \\ 
  \vn{vkicker}                       & D & X & X &   & X &     &  X  &  X  & X \\ 
  \vn{wiggler} (map type)            & D & X & X &   & X &X$^a$&X$^a$&X$^a$& X \\ 
  \vn{wiggler} (periodic type)       &   &   & X &   & X &X$^a$&X$^a$&D$^a$& X \\ \bottomrule
  \multicolumn{10}{l}{$^a$See \sref{s:wiggler.periodic} for more details} \\
\end{tabular}
} 
...
%-----------------------------------------------------------------
\section{Free Electron Laser Method}
\label{s:fel.meth}
\index{fel method}\index{fel_method}

A \vn{wiggler} or \vn{undulator} element can carry a free electron laser (FEL) method, which
names the multiparticle physics a program is to apply within the element:
\begin{example}
  fel_method          = <Switch>   ! FEL method
\end{example}

The possible settings are
\begin{example}
  off             ! No FEL interaction. Default.
  averaged        ! Wiggle-averaged model.
  unaveraged      ! Direct integration of the undulator field.
\end{example}

The setting names what a beam does inside the element and leaves the single particle
\vn{tracking_method} alone, as \vn{space_charge_method} does. A program that computes FEL
physics reads the switch, and every other program tracks the element as the plain periodic
wiggler its field attributes describe. Bmad itself carries the switch and does not act on it.

Example:
\begin{example}
  u1: undulator, l = 3.42, l_period = 0.03, b_max = 1.25, fel_method = averaged, ...
\end{example}
