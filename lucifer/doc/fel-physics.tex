...
\code{lucifer/doc/validation.md}. Each section ends with a \emph{Provenance} note (the Genesis
...
Bmad's units and conventions. The transcription-era agreement is banked in \code{validation.md}.
...
development format \code{lucifer-stats} 2.x. The normative definition of the layout is
\code{doc/BMAD-STATS-SPEC.md} with \code{doc/BMAD-STATS-EXT-FEL.md}, which this section
summarizes rather than restates. Every dataset
...
There is one record axis. An element end is always a record whatever the comb
...
finalize each is propagated to the exit plane by the \code{exp(K2\,dz)} kernel and
the full pulse is written (\code{<out\_root>-pulse.fld.h5}, live window + banked, the
...
\begin{validation}
The coarse-step table in \code{validation.md}, and the
