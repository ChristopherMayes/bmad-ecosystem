...
element-end slice evaluation falls back to Bmad's own
\texttt{calc\_bunch\_params} per slice); zero (the default) records every
...
\texttt{diag} file and the unaveraged ledger follow the comb; on the profiled
fodo cell the diagnostics were 28\% of wall time, so the comb is also the top
