...
\section{The unaveraged mode}\label{sec:unaveraged}

This mode (\code{fel\_unaveraged\_mod}, selected per element by the lattice attribute
...
\code{bmad-stats} 1.0 with the \code{fel} extension, defined normatively by
\code{doc/BMAD-STATS-SPEC.md} with \code{doc/BMAD-STATS-EXT-FEL.md}, and described for
a reader by \code{doc/reading-output.md}. The file is self-describing, so nothing here
restates it.

\emph{Beam statistics.} Per record and per slice the tracker accumulates the
sufficient statistics of \code{bunch\_params\_struct}, named as that struct names
them: \code{centroid}, the \code{sigma} covariance matrix at its natural rank,
\code{charge\_live}, \code{n\_particle\_live}, \code{t} and \code{sigma\_t}. The
moments are computed two-pass, for the reason \S\ref{sec:numerics} gives. Beside them
are the per-coordinate extremes \code{rel\_max} and \code{rel\_min}, taken relative to
...
they are accumulated in the same sweep. \code{current} and \code{energy} in eV follow,
since every consumer would otherwise re-derive them.

\emph{Twiss.} \code{beam/slice\_twiss/} and \code{beam/bunch/} hold the fully
evaluated struct from Bmad's own \code{calc\_bunch\_params}, per slice and for the
...
slice) reconstructs a \code{bunch\_params\_struct}
(\code{scripts/bunch\_params\_from\_stats.py}, transcribing Bmad's
\code{projected\_twiss\_calc} and Wolski's eigen-emittances), and the harness holds
that reconstruction against the stored twiss.

\emph{Slice positions.} The slice axis carries three positions. \code{ct\_slice} and
\code{t\_slice} are exact and free of $\beta$. \code{z\_slice} is Bmad's $z$ at the
reference $\beta$ and says so (Eq.~\ref{eq:tslice} and \S\ref{sec:window}). All three
publish \code{@head\_direction}, the migration invariant (\S\ref{sec:migration}) that
...
\emph{One sweep for two instruments.} The per-record stats loop also evaluates the
\code{diag.txt} instrument, the identical \code{fel\_field\_diag} and
\code{fel\_slice\_diag} calls per slice, so each slice's arithmetic is unchanged and
the diag writer only prints. Every benchmark tier reproduces \code{diag.txt} bit for
...
\code{bunch\_params} reconstruction at $4.6\times10^{-8}$ (checked at $10^{-6}$, the
numpy-vs-\code{mat\_eigen} eigensystem difference, with the projected planes agreeing
...
exactly. \code{diag.txt} is untouched: every benchmark tier reproduces it bit for bit.

The format's own conformance checker, \code{tests/scripts/validate\_bmad\_stats.py},
...
\code{doc/BMAD-STATS-SPEC.md} states and this manual does not restate. Two of its
checks are worth naming here because they guard physics rather than layout. The join:
\code{at\_element\_end} must select exactly the element ends, and every record must sit
inside the element \code{ix\_ele} names, since a silent off-by-one there would break
every layout plot while no physics check noticed. And the record axis: \code{s}
non-decreasing, with every repeated \code{s} straddling an element boundary, since one
plane is reached twice at a boundary while a repeat inside an element would be a defect
in the walk. The zero-length wake lattice is run for that check, where \code{s} really
does repeat, so the check meets its target rather than a run with no duplicates in it.
\end{validation}

\subsection{The whole-window row}\label{sec:pool}
...
\texttt{stats.h5} carries a \texttt{meta/} group: the resolved input echo, the
...
\code{doc/reading-output.md}. Two things about it are design rather than layout.
...
\texttt{lattice/} table, every tracked element with the values the physics used, beside
...
\section{The coherent source (SIMPLEX hybrid)}\label{sec:coherent-source}
