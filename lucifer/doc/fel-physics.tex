...
in one file. A one-byte integer dataset with \code{@unit} of 1 is this format's boolean,
HDF5 having none. Every group carries \code{@kind} and \code{@description} of its own.
...
agrees. Last are the label axes: \code{bmad} and \code{bmad\_col} naming the six phase
space coordinates, \code{wavefront} and \code{wavefront\_col} the four field moments,
\code{plane} the six twiss planes. A square matrix's two sides are named APART so that
selecting one entry needs no rule.
...
element-end axis, with the nine twiss quantities in a \code{twiss/} subgroup, each ONE
dataset over the \code{plane} axis. The planes are an axis rather than six groups: a
group named \code{z} cannot sit beside a \code{z} coordinate, which xarray and netCDF
both refuse, and one array per quantity is 18 datasets where six groups per set were
108. \code{field/} is described below, and \code{meta/} carries the provenance with
\code{lattice/} beside it.
...
serialization: \code{meta/lattice\_file} and \code{meta/lattice\_text} remain the
...
Four structural checks hold the self-description itself. A walk of every dataset for its
...
the walk that demoting \code{z} to a variable would otherwise hide. \code{diag.txt} is
untouched: every benchmark tier reproduces bit for bit.
\end{validation}
