...
\code{z} is a VARIABLE on it: \code{z} repeats wherever two records land on one plane,
so an index on \code{z} would answer silently wrong. \code{ix\_ele} is a variable there
...
element index) and \code{s\_element\_end} (the position). The slice axis is
\code{s\_slice}, with \code{t\_slice} beside it. Both carry \code{@head\_direction},
publishing the migration invariant (\S\ref{sec:migration}) that the HIGH slice index is
the window head: without it a per-slice profile cannot be drawn, let alone compared with
another code's. Stating it on one of a pair would invite the reader to assume the pair
agrees. Both also carry \code{@plot\_against}, naming the variable a
...
Eight structural checks hold the self-description itself. A walk of every dataset for its
...
machine-local value in a default run. \code{diag.txt} is untouched: every benchmark tier
