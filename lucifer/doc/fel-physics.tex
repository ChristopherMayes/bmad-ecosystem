...
carries three attributes: \code{@unit}, \code{@description} and \code{@axes}, the last
naming the \code{coords/} datasets its dimensions run over. Units are fixed Bmad units
(m, rad, eV, s, C, J, W) and the attributes are DOCUMENTATION: the values are already
SI and eV, so a reader must not scale by \code{@unit}. That is the opposite of
openPMD's \code{unitSI} (\S\ref{sec:openpmd}), and the two conventions are deliberately
not mixed in one file. \code{@file\_format\_version} lets a reader dispatch.

Five groups, each answering one question. \code{coords/} holds every axis once: \code{z}
the one record axis, \code{ix\_ele} beside it, \code{at\_element\_end} marking which
records are element ends, and the slice axis \code{s\_slice} and \code{t\_slice}.
\code{s\_slice} carries \code{@head\_direction}, publishing the migration invariant
(\S\ref{sec:migration}) that the HIGH slice index is the window head: without it a
per-slice profile cannot be drawn, let alone compared with another code's.
\code{params/} holds every scalar as data, the window (\code{lambda0},
\code{window\_sample}, \code{slice\_spacing}, \code{nbins}) included, so nothing
downstream parses the echoed namelist. \code{beam/slice/} holds the per-record
SUFFICIENT statistics, named exactly as \code{bunch\_params\_struct} components:
\code{centroid} (nrec, nslice, 6), \code{sigma} (nrec, nslice, 6, 6) at its natural
rank, \code{charge\_live}, \code{n\_particle\_live}, \code{t}, \code{sigma\_t},
...
ELEMENT-END grid. \code{field/} is described below, and \code{meta/} carries the
provenance with \code{lattice/} beside it.
...
\emph{The lattice table} \code{lattice/} carries one row per tracked element, indexed BY
\code{ix\_ele} with element 0 included, which is what a layout plot needs: \code{name},
...
joined through \code{ix\_ele} says the same thing without a second copy of the record
axis, and it says what Genesis cannot: signed quadrupole strengths with a length,
...
a not-computed entry means); unknown dump elements refused by name. Two structural
checks hold the self-description itself: a walk of every dataset for its three
attributes with \code{@axes} naming real \code{coords/} entries, and the join, that
\code{at\_element\_end} selects exactly the element ends and that every record sits
inside the element \code{ix\_ele} names. A silent off-by-one in that join would break
every layout plot while no physics check noticed. \code{diag.txt} is untouched: every
benchmark tier reproduces bit for bit.
\end{validation}
