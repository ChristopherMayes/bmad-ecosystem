...
\emph{The stats file} \code{<out\_root>.stats.h5} DESCRIBES ITSELF. Every dataset
...
\code{lattice/}. \code{at\_element\_end} marks which records are element ends and
selects the \code{element\_end} axis, whose own coordinates are \code{element\_end} (the
element index) and \code{s\_element\_end} (the position). The slice axis is \code{slice}, the
...
\code{plane} the three PROJECTED twiss planes and \code{mode} the three NORMAL modes. A
...
\code{params/} holds every scalar as data, as a true HDF5 scalar, the window
(\code{lambda0}, \code{window\_sample}, \code{slice\_spacing}, \code{nbins}) included,
so nothing downstream parses the echoed namelist. \code{beam/slice/} holds the
...
computed two-pass (the \S\ref{sec:numerics} variance lesson), plus \code{current} and
\code{energy} in eV, which every consumer would otherwise re-derive.
...
Twelve structural checks hold the self-description itself. A walk of every dataset for its
...
$\beta_0\code{ct\_slice}$, and the axis being the index. And one on the zero-length wake
lattice, where \code{s} really does repeat, so the repeated-\code{s} check meets its
target rather than a run with no duplicates in it. \code{diag.txt} is untouched: every benchmark tier
