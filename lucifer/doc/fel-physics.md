...
(sec-quiverdump)=
#### The quiver in a frame

An openPMD momentum record is the instantaneous kinetic momentum: Bmad's
...
$$ u_\perp(s) = \bar u_\perp + a_\perp(x, y, s) $$ (eq-quiverp)

with $\bar u_\perp$ the average the map carries. The element's own potential is
$a_x = a_0\,g(s) f_{aw}\cos\phi$ (planar) and
$(a_x, a_y) = a_0\,g(s) f_{aw} (\cos\phi, \sin\phi)$ (helical), in the wiggle frame,
with $a_0 = \sqrt2 a_w$ planar and $a_w$ helical, $\phi = k_u s_e$ measured from the
element's upstream face, and $f_{aw}$ the roll-off Eq. [](#eq-faw) the map already gathers
with. The two models' transverse profiles agree term for term, so
$\langle a_\perp^2\rangle = (a_w f_{aw} g)^2$ in both.

$g$ is the device's, ends included ([](#sec-unaveraged)). The averaged map integrates a
constant $a_w$ over the whole element and never sees the ends, but what it holds is the
adiabatic invariant a ramped entrance hands it, which is the guiding centre of the ramped
device. A hard edge is the other case, and it is the mode's own mutation configuration:
it starts the quiver about the wrong average and walks the centroid off by $a_0 L/\gamma$.

Position and lag follow by integrating Eq. [](#eq-quiverp) from the face, where the orbit
and the guiding centre coincide:

$$
  \delta x = \frac{a_0 f_{aw}}{u_s} A_c(s), \quad
  \delta y = \frac{a_0 f_{aw}}{u_s} A_s(s), \quad
  A_{c,s}(s) = \int_0^{s}\! g\,(\cos, \sin)(k_u s')\,ds' ,
$$ (eq-quiverx)

$\delta y$ helical only, and $u_s$ the map's own longitudinal momentum, the $\beta_\|$ of
[](#sec-eom) whose $a_w^2$ is already the quiver's mean square. In the body
$A_c = \sin\phi/k_u$ and $A_s = -\cos\phi/k_u$, the quiver's own radius. In a ramp
neither holds: at the middle of a two-period ramp $A_c$ is 13% of the body's amplitude
where $g\sin\phi/k_u$ is zero, so the writer integrates rather than expands. $A_s$ leaves a
constant behind, 6.7% of the radius on the probe line, which is the entrance transient of
a helical device displacing the orbit from its guiding centre. Every integral returns to
zero at the exit face.

The lag is the transverse momentum the longitudinal motion pays for,
$d\tau/ds = \gamma/u_s - 1/\beta_0$ with $u_s$ carrying $u_\perp^2$:

$$
  \delta\tau = \frac{\gamma}{2u_s^3}\Bigl[2\bar u_x a_0 f_{aw} A_c
     + (a_w f_{aw})^2 B_c(s)\Bigr], \quad
  B_c(s) = \int_0^{s}\! g^2 \cos(2k_u s')\,ds' ,
$$ (eq-quivertau)

planar, and $\delta\tau = (\gamma/u_s^3)\,a_0 f_{aw}(\bar u_x A_c + \bar u_y A_s)$
helical, with $z = -\beta\tau$. The $B_c$ term is the figure-8, at twice the undulator
phase, and a helical device has none: its quiver speed does not vary over a period. On the
probe line the two terms are 0.2 and 0.15 rad of optical phase, which is what a frame
without them states wrongly.

What is restored is the oscillation. A ramp also holds $\langle u_\perp^2\rangle$ below the
$a_w^2$ the map carries everywhere, worth 3.29 rad of optical phase per end
([](validation.md)), and that is a difference between the two devices rather than a part of
the orbit. Restoring it would move the frames off the state the run tracked and break the
series at the faces, so it is left alone.

The conversion inverts in the order it is written: the momenta first, which depend on $s$
and the roll-off alone, then $u_s$ from the recovered $\bar u_\perp$, then the position and
the lag. The writer reads the roll-off at the guiding centre, so the inverse reads it at
the record and takes the pass again at the position the first recovered. A planar roll-off
depends on the wiggle frame's $y$, which the quiver does not move, so one pass is already
exact there. A helical one moves with $r$, and the second pass takes that inverse from
second order in the quiver to fourth. `beamio.unquiver` is the inverse, on the frame's own
attributes.
