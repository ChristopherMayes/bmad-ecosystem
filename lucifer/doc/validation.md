...
Ordering the tier blocks, below, took the keystone to 523 to 529 s.
...
The debug pass alone, with no other job running, took 365 s against 692 s under the old schedule inside a keystone, and its concurrent phase took 233 s.

The two passes then reached their tier blocks at the same moment, and eleven single-thread processes each is twenty-two on twelve cores. The block took 216 to 225 s inside a keystone against 115 s alone. The suite now lets the debug pass, which finishes last, through the block first and holds the production pass at its own block until a marker says the first is done (`--tiers-marker` and `--wait-tiers`). The held pass runs its setup meanwhile. The wait is bounded at 900 s and says which way it went, so a pass whose partner died before writing the marker runs its tiers rather than waiting out the keystone. Measured over three keystones on 2026-09-13: the keystone took 523, 525 and 529 s against 573, 579 and 575 s with the blocks overlapping, the debug block 168 to 172 s, the production pass waiting 169 to 173 s and then taking 134 s of its own, and the two passes finishing within 5 s of each other. Mean CPU rose from 1210 to 1218% to 1278 to 1319% of 1600%, and peak resident memory fell from 4.3 GB to 3.4 to 3.5 GB, since eleven trackers now hold the machine at that moment rather than twenty-two.

Widening one section's allowance was measured and rejected. The spontaneous section ends the debug pass's concurrent phase, and its check script's pool, sized from the four cores a section is given, ran its thirteen-run scan one at a time. Given half the pass's cores the section fell from 326 s to 230 to 245 s, and the keystone rose to 626, 637 and 640 s, because a section holding half the allocation leaves room for one other beside it rather than two, and mean CPU fell to 1067 to 1121%. The pass was already spending its allocation: its sections reserved 3984 core-seconds of the 3996 a 333 s phase at twelve cores can hold, so moving cores between them buys nothing and costs the parallelism it takes away. Equal shares stand, and the phase is 328 to 337 s.

The critical path is now the debug pass, at 523 to 529 s of a 523 to 529 s keystone, and inside it the concurrent phase, which the spontaneous section at 318 s still ends. The device lock costs 44 to 49 s of waiting in one pass or the other, inside a phase that ends later than the wait does.
...
What the short-range solve does with a degenerate slice is stated rather than left to the arithmetic. Its grid needs a radial scale: `space_charge%rmax` is a floor under it and the slice's own extent raises it. A slice whose charge sits at one transverse point offers none, and there is no scale to derive, since the cell volumes go as the cell width squared and the field of such a slice has no limit as the grid closes on it. That is not a zero either: particles at one transverse point but different ponderomotive phases push each other, and only a lone particle feels nothing, its own harmonic sum cancelling term by term. The solve therefore refuses when it has no scale, at the slice where the state arises rather than only at setup, since migration reaches it by moving particles out of a slice until one is left. The refusal travels out through the advance to the walk, and the run stops before integrating from a force it did not compute. The solver's grid is also checked where it is stated: a radial grid of fewer than two points, a negative highest azimuthal mode, a negative harmonic count or a negative extent are each refused at setup, where before the first two ran and returned an exact zero with nothing said.

`check_collective.py` holds the five states the solve can meet. A slice with no charge and a slice holding one particle each take exactly zero from the solve, measured 0.0 eV in the mean and the spread against the same run with the master switch off. Two colocated particles at different phases open a spread of 1.1e6 eV while their mean moves a percent of that, which is the statement that the force is mutual and not common. The no-scale case is refused, and so are the two configuration domains.

The long-range term's kernel, $(1 - |d|/\sqrt{d^2 + A})/A$ over the boosted separation $d$ and the slice's transverse area $A$, is evaluated as $1/[\sqrt{d^2+A}(\sqrt{d^2+A} + |d|)]$, the same quantity with the subtraction rationalized away. The two are equal in arithmetic and not in floating point: the first loses every digit once $A$ is small against $d^2$, and its $A$ in the denominator has nothing to cancel against when a slice's charge sits at one transverse point. A guard put a square metre of area there instead, so such a slice radiated orders too little, and migration reaches that state by moving particles out of a slice until one is left. The rationalized form is the limit itself, $1/2d^2$, which carries the whole kernel to $q/4\pi\varepsilon_0 d^2$: the field of a point charge. `check_collective.py` leaves one particle in a slice, isolates its contribution by differencing against the same beam with that slice's charge zeroed, and holds it to that closed form on the three nearest slices at 5e-2, measured 9.9e-4. With the subtraction and the guard restored the same check reads 9.5e-1.
...
What a frame written inside an averaged undulator holds followed (2026-09-14). An openPMD momentum record is the instantaneous kinetic momentum, and the averaged map stores the guiding centre, so the two do not share a record: the frame's particles go to a `.gc.h5` of their own, the map's chart in a file no standard reader opens, whose reductions match the statistics row directly, and a `.beam.h5` is written there only with `global%dump_orbit`, its records the orbit the writer rebuilds from the same near-axis potential this mode integrates: the local vector potential into the momentum, its integral from the upstream face into the position, and the lag that transverse momentum pays for (manual [](fel-physics.md#sec-quiverdump)). The device is the ramped one this mode models, so every term vanishes at an element face and the frames a continuation reads are untouched. This mode is the reference. On a dark eight-period undulator tracked both ways from one beam, with eight frames a period, the part of the residual that oscillates with the undulator phase, which is the part the conversion answers for, is **8e-8** of the quiver planar, **1.6e-5** tilted and **9.5e-5** helical, where a missing $\sqrt2$ in the planar amplitude would leave 0.29 of it and a quarter-period phase origin 1.0. The rest of the residual is the two integrators' guiding centres drifting apart, 1.6e-13 m over 20 periods. Inside a ramp the position and the momentum hold at the same level, and the phase does not: this mode jumps the ramp's phase at the segment's first step while the deficit accrues through the ramp, so the two differ by **2.96 rad** at the entrance, closing to **2.1e-3** by the ramp's end and **6.8e-4** through the body. That is a difference between the two devices and not a part of the orbit, so the conversion leaves it alone. The page's inverse (`beamio.unquiver`, on the frame's own attributes) returns the guiding centre the run tracked to **1.1e-20 m** planar, 2.5e-20 m helical and 1.2e-20 m tilted, on a 2.5e-7 m quiver and against the run's own recorded centroid. The writer reads the roll-off at the guiding centre, so the inverse takes its pass twice, the second at the position the first recovered: a planar roll-off depends on a coordinate the quiver does not move and one pass is exact, and a helical one moves with the radius, where the second pass is worth 1.7e-14 m to 2.5e-20 m. The diagnostics line reads a frame's bunching against the stats row's from the `.gc.h5` with no inverse at all and holds its 2e-12 floor at 1.8e-17, where an exported orbit's own phase moves the bunching by 4.4e-4.

Two limits stay with this, and neither number is equivalence. A converted frame is not an unaveraged trajectory everywhere: inside a ramp the two differ in phase by up to **2.96 rad**, the mode's entry jump standing against the deficit that accrues through the ramp, and nothing the writer puts in the frame closes it. The inverse returns the guiding centre to **1.1e-20 m** and not to the bit, since the conversion adds a number the record then carries and the subtraction cannot undo the rounding. What is established is a reconstruction at the measured approximation order, with the state the run tracked unchanged.

A frame written inside the segment is carried back onto its plane on a scratch copy, and the live record is not touched. The first version carried the live record back and forward again, two transform pairs whose rounding left 5e-15 of the field behind in the continuing run, found by an independent review that compared frames on against off across 48 mixed-line cases: no particle moved, and the field moved by that much. A diagnostic that steers the run it observes by a little is the same failure as one that steers it by more, so the copy is taken, and the whole-window frame of an averaged element keeps its in-place rotation, which is a permutation and exact. `check_unaveraged.py` runs the mixed line with frames every 0.1 m, five of them inside the unaveraged segment, and `check_device.py` the one-plane and two-plane unaveraged decks on the device and the CPU, and each holds the final beam and field identical to the bit against the same run with no frames.
...
| the quiver restored in an exported averaged frame, against this mode's own orbit: planar / helical / planar tilted | **8e-8** / **9.5e-5** / **1.6e-5** of the quiver | 1e-3 |
| the page's inverse of it (`beamio.unquiver`), planar / helical / planar tilted | 1.1e-20 / 2.5e-20 / 1.2e-20 m on a 2.5e-7 m quiver | 1e-16 m |
| the frame series against no frames, the beam a converted frame was copied from | **identical** | to the bit |
| the two modes' frames by centroid momentum, on the diagnostics line | **3.5e-6** | 1e-3 |
| a frame's own bunching against the stats row's, from the `.gc.h5` with no inverse | **1.8e-17** (4.4e-4 from an exported orbit) | 2e-12 |
| the export's inverse against the same frame's `.gc.h5`, and its refusal to guess at a 1.0 frame | 4.2e-22 m, refused | 1e-16 m |
...
That composition check runs steady state, where slippage is a no-op, so a time-dependent restart follows it, across a boundary chosen so the field record has slipped half a slice past its last rotation. What a restart has to work from, traced through the code (2026-09-13):

- Preserved by the files: every particle's coordinates, its lag with the reference phase folded into the file's time, its weight and its id (openPMD's `id` record through `ix_user`), the slice partition through `particlePatches`, and the field's complex records rotated to time order, with the rotation index `first` folded in, plus `slippageResidual`, the signed slippage in fundamental wavelengths not yet rotated. Since 2026-09-14 a dump at an eligible boundary also carries the checkpoint group `lucifer` on both files ([reading-output](reading-output.md)): the reference phase $\varphi_0$ and every particle's $z$ as the tracker holds them, beside the folded records and changing none of them, with the position and element the frames were written at.
- Rebuilt by setup: slice membership from the partition, the start position from the window's entry face, the wavenumber and phase rate from the lattice, and the chamber wake's loss table from the loaded currents, which at an element end are the post-migration currents, since migration runs at an element's last step.
- Reset, as historical totals a continuation does not need: the escaped energy, the spontaneous and radiation sums, the migration and drop counts, and the stats record count.

Migration and the chamber wake were then taken across a restart (2026-09-14). Migration derives a particle's destination from the whole phase, $\varphi_0 + k_s z/\beta$, floored over the window's $2\pi\,$`n_wavelength`, and the dump folds exactly that phase into the file's time, so a decision carries across a restart unless a particle sits nearer a slice boundary than the error in its reconstructed phase. Each row of the migration file therefore records the smallest such distance any particle examined at that event had, the dropped particles included, and the check reads it against the phase error the continuation has actually propagated to the frame before the event, particle by particle by id. On a beam that migrates and drops through three undulators, checkpointed at the first pipe's end after 1569 moves and 6.3e-17 C dropped, with a residual of 2.00004 wavelengths, the three later events had margins of **4.6e-4**, **1.8e-3** and **3.6e-3** rad against propagated phase errors of 3.1e-13 to 7.3e-13 rad, a factor of 1.5e9 or more. With the margin cleared, exact identity holds: ids, slice populations and membership are identical at all 62 frames, every later event matches by position and move count with the dropped charge equal to the digit, and since the checkpoint the moves (1308), the dropped charge and the escaped energy agree, the last to 3e-26 J on 6.0e-13 J at the footer's full precision. The field and energies sit at 6.6e-13 and 1.5e-13. With the chamber wake on, the restart rebuilds the loss table in force at the checkpoint from the surviving charge to the digit, and refreshes it at each of the three later events, at the same positions and to the digit, the same summation running on the same currents on both sides. A separate case shifts every restarted phase by a little less and a little more than the smallest recorded margin: 0.9 of it flips no decision and 1.1 of it moves one particle differently at the first event, which is what the margin means, and that case establishes nothing about equivalence.
Bmad short-range element wakes followed (2026-09-14). Such a wake acts once per passage through the whole window: the concatenation adds each particle's slice offset to its longitudinal coordinate at that particle's own entry $\beta$, Bmad kicks the assembled bunch, and the split subtracts the offsets back, all of it scratch of one passage, so a checkpoint at an element end carries what the next passage needs. What a restart has to place correctly is the kick itself, and the check takes both sides of it: a checkpoint before the wake element, whose continuation must apply the kick, and one at that element's end, whose continuation must not apply it again since the dump already holds it. Which passage applies the kick is read from the runs rather than assumed, as the first frame whose beam departs from a wake-off history, and for a wake a superimposed marker has split onto a lord that is the slave holding the lord's midpoint, `PW#1` of the two. Every restart runs against the same full lattice, so lord ownership is reconstructed identically.

Unaveraged segment boundaries followed (2026-09-14). An unaveraged segment converts on entry and exit, the averaged chart to the kinetic momentum and back, the ramp phase jumped, the field carried a half substep to the midpoint the record advances. None of it outlives the segment, so an element-boundary dump carries the field on its plane and the particles in the averaged chart, and a continuation there runs the conversions once. The continuation at the segment's end held at the floor from the first, **2.6e-12** over 42 frames through the trailing averaged segment. The one at the boundary before the segment did not: its entry field and beam were bit-identical to the continuous run by id, single-threaded, yet it diverged to **6.5e-8** through the segment. The cause is the reference phase. The continuous run enters carrying $\varphi_0$ accumulated to hundreds of radians with a small local lag, the openPMD dump folds the two into one time, and a reader of the records rebuilds an equal phase at a different magnitude, so the deposit phase differs by $\varphi_0$ times the machine epsilon, near 1e-13, which the segment's gain amplifies. The fold is exact for what the file states and no bit of physical beam or field state is lost. What is lost is the tracker's own split of the phase, and the same day a checkpoint group began carrying it: $\varphi_0$ and each particle's $z$ copied from what the tracker holds, in the records' order with the ids and the slice counts, on every dump and element-end frame written at an eligible boundary. A continuation, declared by `global%continuation`, rebuilds the split from the group and refuses anything less than a complete one that agrees with the records. On the same sandwich line the continuation before the segment now rebuilds $\varphi_0$, every $z$, the residual, the records and the field to the bit before any conversion or advance, and holds through the unaveraged segment and the averaged one after it at **1.5e-14** in the field on the worse of the two passes, with every energy and phase equal to the bit, over 82 frames, where the folded records alone reached 6.5e-8. The same checkpoint loaded without the switch is an initialization: the records are folded with $\varphi_0$ at zero, every $z$ moved by up to 1.7e-9 m, and the residual starts at zero, which shows the declaration and not the files' contents controls the loading. Nine continuations are refused before tracking, each message naming its condition: a frame from inside the unaveraged segment, a frame from inside an averaged undulator, a group with a member removed, the group removed, the residual removed from the field file, a beam frame and a field frame from two positions, coordinates permuted against their records, a version this tracker does not know, and a `track_start` one element past the checkpoint's. A sliced endpoint inside an undulator, the end of a super_slave a superimposed element has cut, is the one ineligible boundary no run reaches: the slaves refer to their lord for the field model and the lattice is refused at setup, which the check records, while the eligibility test guards that boundary for the day a cut undulator is tracked. The `momentumChart` attribute a beam file carried for one day was withdrawn with the group's arrival: it labelled the tracker's internal momentum convention onto a standard record, where openPMD momenta are kinetic and a frame from inside the segment already holds them, and the refusal it carried now rests on the group's absence.

The wake-off history runs from the same initial state and never from a wake-on dump, which already holds the kick that no continuation can undo, and a wake-off restart takes the wake-off run's own checkpoint. The wake's effect is the scale the agreement is measured on, since the beam's total energy would hide a small kick: the wake moves the energies by **4.0e-4** of themselves, and each restart reproduces that effect to **1.3e-9** of it on the element and to **3.3e-13** through the lord, with ids, populations and membership identical at every frame and the fields, energies and phases at 2.2e-12, 5.2e-13 and 3.8e-13. Migration is off in these cases, so the wake is the only configuration that differs between the paired runs, transport and the FEL interaction acting in all of them. Two deliberate failures, measured before the continuation point became part of the contract, fixed what the scale buys: a continuation restarted at the wake element from the post-wake dump applied the kick twice, and one restarted at the following element from the pre-wake dump omitted it. The first moved the energies by 4.0e-4 again and the second reproduced exactly **1.00** of the effect, where an agreeing restart sits nine orders below. Since the checkpoint group records the element a checkpoint completed, such a continuation is refused before tracking: `track_start` must open the walk at the element right after it, at its position. Long-range and history-dependent wakes are refused at setup and stay so.

Until 2026-09-13 the residual was in the third list, and it belongs in the first: started at zero, a restart's every later threshold crossing moves by the lost amount. The check (`check_program.py`) runs two averaged undulators with a pipe between, four wavelengths a slice, a seed ramped slice by slice so a rotation in the wrong place shows, and the beam and field imported so no loader noise enters. The dump at the pipe carries a residual of **2.00004** wavelengths of the four a slice holds. The restart reproduces the continuous run's 31 frames through the second undulator, field slice by slice in time order and particles matched by id, to **1.8e-12** in the field, **1.2e-13** in phase, **1.0e-12** in energy and **1.0e-12** in the power rows, with ids and slice populations identical, held at 1e-10. Measured before the loading modes were declared (2026-09-13), a restart from the same dump with the attribute stripped, which is what a file written before the attribute existed is, rotated on its own schedule: its field differed by **0.59** from the second step of the second undulator, where the continuous run crosses the threshold and the zero-started one did not, and its phases and energies drifted to 5.2e-7 and 1.2e-5 by the exit. A pipe 5 cm longer leaves a residual of **3.8e-5** wavelengths, and there the stripped restart agreed to 2.0e-12 as well, which is why the steady-state composition and every whole-slice boundary before it never saw the loss. A continuation from a stripped dump is now refused in both cases: the residual is part of the field's state, and a continuation takes nothing from a file that lacks it, while an initialization from the same file starts the record at zero whatever it carries. The escaped energy's increment past the pipe agrees with the restart's total within the footer's six printed digits, which is the resolution that footer offers. A residual of 7 wavelengths written into the attribute is refused.
