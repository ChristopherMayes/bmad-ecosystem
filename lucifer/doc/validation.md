...
and the same four checks hold on the device run at the device's grid, the no-op
self-referenced against the device's own flutter since its deposit is not byte-reproducible
([](#val-device)).
...
| wakes, radiation, harmonics together with two polarizations, the unaveraged mode, an unknown backend, an unsupported grid | each refused |
...
| migration no-op on the device: moves; migrate T vs F final beam [slice spacings] | 0; <= 3 x flutter or 1e-12 (measured 1.7e-14 against a flutter of 1.1e-14) |
