...
Two live polarizations inside this mode is the one configuration the CPU carries and the device refuses. Harmonics, wakes, space charge and the coherent source are refused for the mode on both. `fp32_check = "freerun"` stays refused for the mode, since freerun compounds a single-precision state across steps and this twin carries none.
