...
Unaveraged segment boundaries followed (2026-09-14). An unaveraged segment converts on entry and exit, the averaged chart to the quiver and back, the ramp phase jumped, the field carried a half substep to the midpoint the record advances. None of it outlives the segment, so an element-boundary dump carries the field on its plane and the particles in the averaged chart, and a restart there runs the conversions once. Two contracts hold and one is deferred. A restart at the segment's end continues through the trailing averaged segment at the floor, **2.6e-12** over 42 frames. A restart from a frame written inside the segment, which holds quiver-carrying momenta, is refused: the beam file records the chart in `momentumChart`, and the reader rejects a `quiver` file where the `felMethod` label alone could not, since an element-end frame reads unaveraged with the chart already handed back. A restart at the boundary before the segment is not yet at the floor. Its entry field and beam are bit-identical to the continuous run by id, single-threaded, yet it diverges to **6.5e-8** through the segment. The cause is the reference phase: the continuous run enters carrying $\varphi_0$ accumulated to hundreds of radians with a small local lag, the openPMD dump folds the two into one time, and the reader rebuilds an equal phase at a different magnitude, so the deposit phase differs by $\varphi_0$ times the machine epsilon, near 1e-13, which the segment's gain amplifies. The fold is exact for external readers and no bit of physical beam or field state is lost, so this is the fold's cost to a high-gain segment and not a relaxed tolerance. Carrying the reference phase and the local lag losslessly in versioned Lucifer metadata beside the folded records resolves it, and until then the before-segment guarantee is unresolved.

The wake-off history runs from the same initial state and never from a wake-on dump, which already holds the kick that no continuation can undo, and a wake-off restart takes the wake-off run's own checkpoint. The wake's effect is the scale the agreement is measured on, since the beam's total energy would hide a small kick: the wake moves the energies by **4.0e-4** of themselves, and each restart reproduces that effect to **1.3e-9** of it on the element and to **3.3e-13** through the lord, with ids, populations and membership identical at every frame and the fields, energies and phases at 2.2e-12, 5.2e-13 and 3.8e-13. Migration is off in these cases, so the wake is the only configuration that differs between the paired runs, transport and the FEL interaction acting in all of them. Two deliberate failures fix what the scale buys: a continuation restarted at the wake element from the post-wake dump applies the kick twice, and one restarted at the following element from the pre-wake dump omits it. The first moves the energies by 4.0e-4 again and the second reproduces exactly **1.00** of the effect, where an agreeing restart sits nine orders below. Long-range and history-dependent wakes are refused at setup and stay so.

Until 2026-09-13 the residual was in the third list, and it belongs in the first: started at zero, a restart's every later threshold crossing moves by the lost amount. The check (`check_program.py`) runs two averaged undulators with a pipe between, four wavelengths a slice, a seed ramped slice by slice so a rotation in the wrong place shows, and the beam and field imported so no loader noise enters. The dump at the pipe carries a residual of **2.00004** wavelengths of the four a slice holds. The restart reproduces the continuous run's 31 frames through the second undulator, field slice by slice in time order and particles matched by id, to **1.8e-12** in the field, **1.2e-13** in phase, **1.0e-12** in energy and **1.0e-12** in the power rows, with ids and slice populations identical, held at 1e-10. A restart from the same dump with the attribute stripped, which is what a legacy file is, rotates on its own schedule: its field differs by **0.59** from the second step of the second undulator, where the continuous run crosses the threshold and the zero-started one does not, and its phases and energies drift to 5.2e-7 and 1.2e-5 by the exit. A pipe 5 cm longer leaves a residual of **3.8e-5** wavelengths, and there the stripped restart agrees to 2.0e-12 as well, which is why the steady-state composition and every whole-slice boundary before it never saw the loss. The escaped energy's increment past the pipe agrees with the restart's total within the footer's six printed digits, which is the resolution that footer offers. A residual of 7 wavelengths written into the attribute is refused.
