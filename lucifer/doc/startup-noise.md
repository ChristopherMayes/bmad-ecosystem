...
At the end of the first segment the power outside the cut is independent of the particle count and increases as the cells shrink, as $1/dx^2$ over the first halving of the cell and more slowly over the next two ([](#tab-sn-floor), [](#fig-sn-split), [](#fig-sn-scaling)): the exponent is 2.25, 1.42 and 1.08 over the three halvings. The number of transverse modes the grid carries scales as $1/dx^2$, and a deposit alone follows that exponent whatever its shape function, so the shortfall arises between the deposit and this plane. It is the coherence of a wide-angle mode along $z$. The solver propagates the field between deposits, and a mode at angle $\theta$ slips against the axial wave by $k_s\theta^2\Delta z/2$ per step, the paraxial form of the angular red shift. The field record slips one slice past the beam every $s\lambda_u$ of undulator, $s$ being the slice spacing in wavelengths, so a field slice receives the deposits of one static beam slice for $N_{\rm ref} = s\lambda_u/\Delta z$ steps and of an independent one after, here four steps. A block of $N$ deposits adds as $\sin^2(N\phi/2)/\sin^2(\phi/2)$, which is $N^2$ while the block is coherent and $N$ once it is not, so every mode of a coarse grid counts fully and the exponent is 2, and the modes beyond the knee at $\theta_t = \sqrt{\lambda/(s\lambda_u)}$, 23.6 µrad here, count $1/N_{\rm ref}$ of that. That angle sits between the third and fourth grids' Nyquist angles of 31.9 and 63.9 µrad, where the exponent starts to fall. The sum over the grid's own modes, with nothing fitted, gives 2.26, 1.47 and 0.90.

```{figure} generated/startup-noise/cell-size-cause.png
:name: fig-sn-cause

The exponent of the wide-angle power at the first segment's end against the derivation, on one segment at 1024 macroparticles. Top left: the floor against the cell size, with the derived sum scaled at 1.57 µm and $1/dx^2$ dashed. Top middle: the exponent over the 3.15 to 1.57 µm halving against the slice spacing, which moves the knee as $s^{-1/2}$. Top right: the same exponent over the 1.57 to 0.78 µm halving against the step at fixed spacing, which moves the contrast above the knee and returns the exponent to 2 at one step a rotation. Bottom left: the exponent against the half width, which does not enter. Bottom middle: the wide-angle power against $z$ with the derived sum and a line through the origin. Bottom right: the power after each of the first thirteen steps, with the slice rotations marked.
```

Four sweeps test it, each moving one thing ([](#fig-sn-cause)). The half width at 200, 400 and 800 µm leaves the exponent over the 3.15 to 1.57 µm halving at 1.42, 1.41 and 1.41, so the window is not in it. The slice spacing at 3, 6, 12, 24 and 48 wavelengths moves the knee, that exponent reading 2.06, 1.84, 1.42, 1.01 and 0.74 against 2.07, 1.90, 1.47, 0.98 and 0.63 derived. The step at 22.5, 44.8, 89.7 and 179 mm leaves the first two exponents within 0.04 and moves the third from 0.65 to 2.04, and at one step a rotation the exponent is 2 at every cell. Along the segment the wide-angle power is linear in $z$ to one percent of the derived sum, and over the first thirteen steps it rises by 5.3, 6.3, 6.7 and then 2.2 MW per slice three times over, the fourth step of each block being the rotation. The derivation reads 0.1 to 0.25 low on the finest halving, where the incoherent floor is a little higher than a block of identical deposits gives. The sweeps and the sum are experiment i of `tests/scripts/startup_noise.py`, and a second pass over its work directory reproduces every number.
...
$$\frac{P_{wide}}{P_{mode}} \approx \frac{90}{N_b}\,F(dx),\qquad F = 0.078,\ 0.37,\ 1,\ 2.1 \text{ at } dx = 6.35,\ 3.15,\ 1.57,\ 0.78\ \mu\mathrm{m},$$
where $N_b$ is the number of beamlets per slice and $F$ is the floor's own dependence on the cell size, [](#tab-sn-floor) relative to its 1.57 µm row, which the derived sum above reproduces to 12 percent. The fit is over cells of 0.78 to 6.35 µm and 128 to 2048 beamlets at the eighth undulator's end, with the run at 0.78 µm and 128 beamlets set aside since the artifact drains that beam, and its residual is 0.77 to 1.56. The $(1.57\,\mu\mathrm{m}/dx)^2$ this page carried fits to 0.44 to 1.38, high where the knee has passed. The prefactor is for the beam of [](#tab-sn-beam) and scales with the bunching factor squared. At saturation the wide-angle power's exponent also depends on the beamlet count, 2.5, 2.0 and 1.5 over the three halvings at 128 beamlets against 1.6, 1.2 and 1.0 at 8192, so beyond 2048 beamlets the factor overstates the ratio by up to three, which is the direction a criterion may err in. It does not carry to another machine: on FLASH1 at 13.7 nm the same expression is 40 times low over cell sizes of 3.9 to 31 µm. The form is the guide for choosing a load before a run, and the number that decides the question is the one the run reports. Keeping the ratio below a tenth requires $N_b > 1000\,F(dx)$, which puts the measured ratio at 0.04 to 0.09 at every cell: 373 beamlets at 3.15 µm and 1000 at 1.57, or 2984 and 8000 macroparticles per slice at a beamlet size of 8.
