...
The beam and the undulator line are those of the Aramis benchmark distributed with Genesis4, which the examples in this documentation also use ([](#tab-sn-parameters)). The line is six FODO cells, each holding two helical undulator segments of 266 periods, two quadrupoles and four drifts, for twelve segments and 57 m in all.

```{table} Beam, undulator and simulation parameters. The beam is matched to the periodic focusing of the cell.
:name: tab-sn-parameters

| quantity | value |
|---|---|
| beam energy | 5.80 GeV ($\gamma$ = 11358) |
| peak current | 3.0 kA, uniform along the window |
| normalized emittance, both planes | 0.40 µm |
| rms relative energy spread | $8.8\times10^{-5}$ ($\sigma_\gamma$ = 1.0) |
| matched beta functions at the cell entrance | 8.54 m and 17.4 m |
| rms beam size at the mean beta function | 21 µm |
| undulator | helical, period 15.0 mm, rms parameter $a_w$ = 0.849 |
| segment | 3.99 m, 266 periods, 12 segments |
| FODO cell | segment, 0.44 m drift, 0.08 m quadrupole with $k_1 = \pm 2.0$ m$^{-2}$, 0.24 m drift, twice, 9.5 m in all |
| radiation wavelength | 1.00 A |
| integration step | 45 mm, three periods |
| field grid | 200 µm half width, 64 to 512 points, cells of 6.35 to 0.78 µm |
| macroparticles per slice | 1024 to 65536, in beamlets of 8 |
| Pierce parameter $\rho$ | $5.1\times10^{-4}$ |
| cooperation length $\lambda/(4\pi\rho)$ | 16 nm |
| slippage over the line | 320 nm |
| central cone of one segment | 7.1 µrad |
...
within a factor of 1.5 over cell sizes of 0.78 to 6.35 µm and beamlet counts of 128 to 2048, where $N_b$ is the number of beamlets per slice and $dx$ the cell size. The prefactor is for the beam of [](#tab-sn-parameters) and scales with the bunching factor squared. Keeping the ratio below a tenth requires $N_b > 1000\,(1.57\,\mathrm{µm}/dx)^2$, which is 4096 macroparticles per slice at a beamlet size of 8 for 3.15 µm cells and 16384 for 1.57 µm cells.
