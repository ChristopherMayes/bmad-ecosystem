...
| `global%source_filter_tolerance` | `0.75` | The largest fraction of source intensity the filter may take inside the protected angle |
...
| `global%device_timing` | `F` | Time each pass of a device step separately, one encoder a pass ([](#param-global-device-timing)) |
...
(param-global-device-timing)=
**`global%device_timing`** times each pass of a device step separately. The backend's own busy seconds come from one command buffer, and a command buffer holds every step between two host touches, so they say what a step cost and not what a pass cost. On, each pass is encoded into its own compute encoder carrying a timestamp counter attachment, still inside the one command buffer the step batching rests on, and the run reports seconds and encoder counts for the transverse map, the push, the source clear, the deposit, the filter and the solve. The passes of one kind sum into one line, so the transverse map reports two encoders a step and the filter and the solve four each. Stage-boundary counter sampling is what an Apple GPU carries, and it times an encoder rather than a dispatch, which is why a pass gets an encoder of its own; a device that samples nothing at an encoder boundary refuses rather than reporting zeros. The instrument therefore pays encoder boundaries the production path does not, measured at 1.2 to 2.6 times the device busy seconds, so it is off by default and a price is taken with it off ([](performance.md#perf-device)). The sample buffer holds 2048 passes, about 157 steps, and a command buffer needing more is committed early with a warning saying how often, since that is a batching change the instrument caused.
...
| `global%interlude_ds_step` | `0` | Cut an interlude element into pieces of about this length [m], so rows and frames land along it. Zero tracks it whole |
...
**`global%dump_at_comb`** writes the beam and the field set at every position the stats comb takes a row, as `<out_root>-<record>.beam.h5` and `<out_root>-<record>.wf.h5` with the record zero padded to six digits. That is openPMD's fileBased series, and the index is the stats record's, so a frame and the row describing it are read on one axis. The writers are the ones the element-end dumps use, so a frame taken at an element end is the same file that element's `dump_beam_at` entry would write. `comb_ds_save` therefore sets the frame spacing, and a comb of zero is a frame at every integration step, which on a long line is tens of gigabytes: the header's `Frames` line states the count and the bytes before the run tracks. A frame inside an unaveraged segment is written in the chart that mode tracks in, with the undulator quiver in `px` and `py`, and the frame's `felMethod` says so. The averaged physics and the Bmad seam still refuse that chart, since they would read it as the averaged one, and the beam dump of `dump_beam_at` keeps that refusal because it writes no `felMethod` to distinguish them. It never bites: `dump_beam_at` names elements through Bmad's locator and so lands on element boundaries, where the ramps have handed the averaged convention back.
...
**`global%comb_ds_save`** is Bmad's `bunch_track_struct%ds_save` name and semantics. Negative keeps no per-record rows at all, while element ends, dumps and the final state remain. Zero, the default here, records every position the walk reaches. Positive records a row once z has advanced that far past the last one, with element ends always kept. The default deviates from Tao's, deliberately: the per-record arrays are this program's primary statistics contract where Tao's comb is an optional extra.

Note: this is the selecting half of Tao's comb and not the whole of it. `save_a_bunch_step` selects among the positions a tracker already reaches, and that is what this setting does. Tao's beam-track loop also cuts an element into pieces where the tracker would otherwise cross it in one map, and here that is `interlude_ds_step` below. An FEL element reaches one position per `ds_step`, so this setting alone gives a fine comb inside a wiggler. An interlude is one tracking call over the whole element, so this setting alone gives one row there, at its end, whatever it is set to.

**`global%interlude_ds_step`** cuts an interlude element into pieces of about this length, so statistics rows and openPMD frames land along a drift or a quadrupole the way they already land along a wiggler. Zero, the default, tracks the element whole, which is what every run did before this setting existed and what the recorded validation digits are taken at. The pieces come from Bmad's own `element_slice_iterator`, the routine Tao's comb uses, so an element's fringes stay at its own ends rather than appearing at every cut. The count is Tao's expression, `max(1, int(1.01 * L / ds))`, so an element shorter than the step is one piece and the run's header states the total before it tracks. Each piece tracks the beam, drifts the field, advances the common phase and takes its share of the element's slippage, so the element's exit is the element's exit however many pieces crossed it ([](validation.md#val-interlude-steps)). A chicane's chord-versus-arc correction is a property of the whole break and lands on its last element's last piece. Two configurations decline the cut. An element carrying a Bmad short-range wake is tracked whole, since that wake is a once-per-passage kick of the element's length and would otherwise become one kick a piece, which is the judgment Tao makes where CSR or space charge would see a cut. `interlude_model = "genesis"` refuses the setting outright, that model being a transcription whose step is Genesis's own element.
...
**`wavefront_init%grid_n_pts`** and **`wavefront_init%grid_half_width`** are derived from the beam when the deck leaves them at zero, and both are printed with their origin at setup. The rms beam size comes from the emittances the deck states and the matched Twiss the lattice states, averaged over the FEL elements by length. The half width is nine of those, which contains the mode, and the cells are a seventh of one, which resolves the beam and the mode. Both constants were measured on three machines a hundred times apart in wavelength ([](startup-noise.md#sn-recommendations)), and the two together fix the point count at 127 before rounding, since the beam size cancels. That count is then rounded up to the next power of two, 128, so that one grid serves the CPU and the device field solver alike and a derived deck moves between the backends unchanged. Rounding up only ever refines, more points across one half width being smaller cells, and setup prints the rounded count beside the count it came from.

One may be set and the other derived against it. A stated half width keeps the cell rule, so the point count follows from it. A stated point count keeps the containment rule, so the half width is nine beam sizes and the cells are whatever the count makes them. Nothing is derived without an emittance to derive from: a run that loads its beam from a dump states its own grid, or takes the field's from `field_file`. A field from `field_file` is read with its own `unitSI` and `gridUnitSI` applied, its `y` component must be shaped as its `x` is, and the source filter's angle is converted on the file's grid rather than on any grid the deck states.
...
`fft_3d` and `cathode_fft_3d` are refused on an FEL element, since their solvers want a three-dimensional grid this walk does not build. On an element that is not an FEL element, any method other than `off`, and any `csr_method` other than `off`, is refused while the master switch is on and the interlude model is `bmad`: the seam tracks one slice at a time through Bmad's `track1_bunch`, whose collective paths need a centroid orbit this program does not carry, and Bmad would return the bunch untracked with no error. The transcribed Genesis interlude applies the slice solve itself, so with `interlude_model = "genesis"` a `slice` interlude is tracked. Bmad's master switch applies as it does everywhere else: `bmad_com%csr_and_space_charge_on` must also be true, and when elements ask for `slice` while it is false the run says so and tracks without space charge. `space_charge_com%n_bin` is ignored, because the slices are the bins. Inside the Bmad seam the same attribute drives Bmad's own machinery, so one lattice reads the same way in every Bmad program.
