...
The run reports the split between the mode and the wide-angle emission of the point beamlets, which is the emission of macroparticles that occupy one grid point each and is no part of what a real beam radiates ([SASE convergence](../../startup-noise.md)). At the exit the power outside the split angle is 0.9 percent of the power inside it, so the 26.7 MW above is the mode. With the filter off the same deck reported 2.94 GW, of which 99 percent was that emission. The wake's effect is measured on the beam and on the energy panel, which the split does not touch.
