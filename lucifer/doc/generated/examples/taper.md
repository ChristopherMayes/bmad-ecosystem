...
The run reports the split between the mode and the wide-angle emission of the point beamlets, which is the emission of macroparticles that occupy one grid point each and is no part of what a real beam radiates ([SASE convergence](../../startup-noise.md)). At the exit the power outside the split angle is 0.08 percent of the power inside it on the tapered line, so the 8.85 GW is the mode and the taper's gain is the mode's. With the filter off the same deck reported 9.64 GW, of which 9 percent was that emission.
