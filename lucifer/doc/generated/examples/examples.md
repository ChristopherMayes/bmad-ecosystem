...
| [`frames/`](frames.md) | A frame series along z over a slice range, with the field's reductions | 0.4 s |
...
| [`flash1/`](flash1.md) | FLASH1 at DESY at 13.7 nm from its published parameters, against its measured gain length and pulse energy | 20 s |
| [`lcls/`](lcls.md) | LCLS at SLAC at 1.5 Angstrom from its published parameters, against its measured gain length and saturated power | 90 s |
...
| openPMD dumps in both directions | [`steady_state/`](steady_state.md) writes mid-run, [`import/`](import.md) round-trips a bunch through a file, [`frames/`](frames.md) writes a series along z, [`saturation_demo/`](saturation_demo.md) reads Genesis4's |
