...
The run reports the split between the mode and the wide-angle emission of the point beamlets, which is the emission of macroparticles that occupy one grid point each and is no part of what a real beam radiates ([SASE convergence](../../startup-noise.md)). At the exit the power outside the split angle is 1.7 percent of the power inside it in both runs, so the exit powers here are the mode. With the filter off both decks reported a hundred times more power, 27 times the mode's, all of it that emission. The comparison between the two wake implementations is made on the beam's energy loss, which the split does not touch.
