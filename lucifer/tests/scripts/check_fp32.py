...
1. Lockstep levels. A steady one-segment run with fp32_check = "lockstep" must land
   its worst per-quantity divergence inside the recorded ceilings, its residual guard
   must stay in healthy ulps, and the bucket-renormalization round trip must be exact
   to one ulp.
...
3. Falsifiable. fp32_mutate = T coarsens the FP32 residual by eight mantissa bits, and
   the recorded theta level must move by an order of magnitude, so a wrong twin cannot
   hide behind a check that always passes.
4. Compounding. freerun (the FP32 longitudinal state carried across steps) must
   diverge past lockstep on the phasor: refreshing from FP64 each step is what keeps
   lockstep's levels flat, and the difference is the compounding rate.
...
CEIL = {"pz": 2e-6, "theta": 3e-4, "phasor": 3e-5}
GUARD_FLOOR = 256.0
RENORM_CEIL = 1.5
...
def main():
    ...
    # 3. Falsifiable: the mutation must move the theta level.
    ...
    # 4. Compounding: freerun past lockstep on the phasor.
