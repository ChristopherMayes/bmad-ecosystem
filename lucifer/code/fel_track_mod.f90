...
type fel_coherent_struct
  ...
  real(rp) :: m_ind = 1                  ! Independent transverse samples, the slice's load-time count.
...
!------------------------------------------------------------------------------
!+
! Function fel_und_envelope (l, l_ramp, s, gp) result (g)
!
...
if (s < l_ramp) then
  arg = pi * s / (2 * l_ramp)
  g = sin(arg)**2
  gp = (pi / l_ramp) * sin(arg) * cos(arg)
elseif (s > l - l_ramp) then
  arg = pi * (l - s) / (2 * l_ramp)
  g = sin(arg)**2
  gp = -(pi / l_ramp) * sin(arg) * cos(arg)
endif

end function fel_und_envelope

!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
!+
! Subroutine fel_restore_quiver (und, beam, l_ele, ramp_periods, s_ele)
!
...
real(rp) l_ele, ramp_periods, s_ele, lam_u, l_ramp, g_here, gp_here, a0, p0_mc, phi
real(rp) a_cos, a_sin, b_cos, p_mc, gam, beta, awloc, amp, aw_loc
real(rp) ux, uy, ut, us, dux, duy, dx, dy, dtau, f_pos, f_tau
integer is, ip

! The phase and the peak potential are the element's and not the particle's. aw is the rms
! undulator parameter, so the peak is sqrt(2) aw planar and aw helical.

lam_u = twopi / und%ku
l_ramp = ramp_periods * lam_u
phi = und%ku * s_ele
g_here = fel_und_envelope(l_ele, l_ramp, s_ele, gp_here)
a0 = und%aw
if (.not. und%helical) a0 = sqrt(2.0_rp) * und%aw
p0_mc = fel_p0_mc(beam)
call phase_integrals (a_cos, a_sin, b_cos)
...
do is = 1, size(beam%slice)
  ...
  do ip = 1, sl%n
    ...
    ! The wiggle frame, where the potential is written. sin_t = 0 skips both rotations.

    ux = sl%px(ip) * p0_mc
    uy = sl%py(ip) * p0_mc
    if (und%sin_t /= 0) then
      ut =  und%cos_t * ux + und%sin_t * uy
      uy = -und%sin_t * ux + und%cos_t * uy
      ux = ut
    endif

    ! The map's own longitudinal momentum, btpar of fel_track_und_step: the quiver's mean
    ! square is already in it, and only the oscillating part is missing.

    us = sqrt(gam**2 - 1 - ux*ux - uy*uy - aw_loc*aw_loc)
    f_pos = amp / us
    f_tau = gam / (2 * us**3)

    dux = amp * g_here * cos(phi)
    dx  = f_pos * a_cos

    if (und%helical) then
      duy = amp * g_here * sin(phi)
      dy  = f_pos * a_sin
      dtau = f_tau * 2 * amp * (ux * a_cos + uy * a_sin)
    else
      duy = 0
      dy  = 0
      ! The second term is the figure-8, at twice the undulator phase. A helical device
      ! has none: its quiver speed does not vary over a period.
      dtau = f_tau * (2 * ux * amp * a_cos + aw_loc * aw_loc * b_cos)
    endif

    if (und%sin_t /= 0) then     ! Back out of the wiggle frame.
      ut  = und%cos_t * dux - und%sin_t * duy
      duy = und%sin_t * dux + und%cos_t * duy
      dux = ut
      ut  = und%cos_t * dx - und%sin_t * dy
      dy  = und%sin_t * dx + und%cos_t * dy
      dx  = ut
    endif
    ...
    sl%x(ip)  = sl%x(ip) + dx
    sl%y(ip)  = sl%y(ip) + dy
    sl%px(ip) = sl%px(ip) + dux / p0_mc
    sl%py(ip) = sl%py(ip) + duy / p0_mc
    sl%z(ip)  = sl%z(ip) - beta * dtau
  enddo
enddo

!------------------------------------------------------------------------------
! The phase integrals from the upstream face.

contains

!+
! Subroutine phase_integrals (a_cos, a_sin, b_cos)
!
...
real(rp) a_cos, a_sin, b_cos, h, ss, gg, gpp, wt, ph
integer i, n

!

a_cos = 0;  a_sin = 0;  b_cos = 0
n = max(64, 2 * nint(128 * s_ele / lam_u))
h = s_ele / n
...
do i = 0, n
  ...
  if (mod(i, 2) == 1) wt = 4
  if (i == 0 .or. i == n) wt = 1
  gg = fel_und_envelope(l_ele, l_ramp, ss, gpp)
  ph = und%ku * ss
  a_cos = a_cos + wt * gg * cos(ph)
  a_sin = a_sin + wt * gg * sin(ph)
  b_cos = b_cos + wt * gg * gg * cos(2 * ph)
enddo
...
end subroutine phase_integrals

end subroutine fel_restore_quiver
...
real(rp) g_max, p0_mc_l, s_bound, r_max
...
! The deposit's fixed-point scale. The bound is the charge the run loaded over the
...
! the mesh carries. fel_device_dep_scale turns that into a scale and refuses a bound it
! cannot carry.
...
call fel_device_dep_scale (dev, s_bound, 'the run''s charge at an eighth of its lowest ' // &
        'loaded gamma, the element''s source scale and the roll-off at the box corner', &
        par%dep_scale, err)
if (err) return
...
coh%m_ind = max(1.0_rp, sl%m_ind)      ! Independent transverse samples, counted at load.
coh%ok = .true.
