...
The run reports the split between the mode and the wide-angle emission of the point beamlets, which is the emission of macroparticles that occupy one grid point each and is no part of what a real beam radiates ([SASE convergence](../../doc/startup-noise.md)). At saturation the power outside the split angle is 0.08 percent of the power inside it, and at the exit 0.24 percent, so the 792.7 MW quoted above is the mode. With the filter off the same deck reported 761.5 MW at the exit and 1.62 GW at saturation, of which 45 and 9 percent were that emission.
