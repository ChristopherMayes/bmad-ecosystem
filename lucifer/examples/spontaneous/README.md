...
The run reports the split between the mode and the wide-angle emission of the point beamlets, which is the emission of macroparticles that occupy one grid point each and is no part of what a real beam radiates ([SASE convergence](../../doc/startup-noise.md)). At saturation the power outside the split angle is 0.13 times the power inside it and at the exit 0.81 times, so 45 percent of the exit power is that emission. The damping and fluctuation measurements are made on the beam's energy, which the split does not touch.
